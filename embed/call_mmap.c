...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4)
    {
        fprintf(stderr,"Usage: call_mmap python_file class_name function_name\n");
        return 1;
    }
...
#else
    int fd;
    //if((fd = open("input.dat", O_RDWR | O_CREAT)) == -1)
    if((fd = open("input.dat", O_RDWR)) == -1)
    {
        printf("Couldn't open 'input.dat'\n");
    }
    ...
    if(indata != NULL)
    {
        printf("Wrapper has created a MMAP for file 'input.dat'\n");
    }
